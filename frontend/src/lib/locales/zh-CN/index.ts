export const zhCN = {
  ...
  apiErrors: {
    ...
    embeddingModelRequired: "此功能需要智能检索模型。请在模型设置中配置一个嵌入模型。",
  ...
  connectionErrors: {
    ...
    apiDesc: "无法访问 ForgeNote API 服务器",
    dbTitle: "数据库连接失败",
  ...
  sources: {
    ...
    sourceRemovedFromNotebook: "来源已从当前学习记录移除，原来源仍保留在资料库中。",
    failedToRemoveSourceFromNotebook: "从当前学习记录移除来源失败",
    removeConfirm: "确定要从当前学习记录移除此来源吗？这不会删除来源本体，之后仍可从资料库重新添加。",
    checking: "正在检查...",
    ...
    selectMultipleFilesHint: "选择多个文件进行批量导入。支持：文档 (PDF, DOC, DOCX, PPT, XLS, EPUB, TXT, MD)，媒体 (MP4, MP3, WAV, M4A；配置语音识别模型后会自动转写)，图片 (JPG, PNG)，压缩包 (ZIP)",
    speechRecognitionTitle: "语音识别",
    speechRecognitionHint: "上传音频或视频文件后，系统会使用已配置的语音转文字模型转写，再作为学习来源参与检索和资产生成。",
    selectedFiles: "已选择文件：",
    ...
    addSource: "添加来源",
    notEmbeddedAlert: "尚未建立智能检索索引",
    notEmbeddedDesc: "建立索引后，AI 问答和学习资产生成可以更准确地检索这份来源内容。",
    ...
    enableEmbedding: "让 AI 可检索此来源",
    embeddingDesc: "建立智能检索索引，便于搜索、问答和资产生成使用。",
    embeddingAlways: "已自动建立智能检索索引",
    embeddingAlwaysDesc: "您的设置已配置为新内容自动建立智能检索索引。",
    embeddingNever: "未启用智能检索索引",
    embeddingNeverDesc: "您的设置已配置为跳过索引。此来源仍会保存，但 AI 检索效果会受限。",
  ...
  chat: {
    ...
    voiceInput: "语音输入",
    voiceInputListening: "正在听写，点击停止",
    voiceUnsupported: "当前浏览器不支持语音输入，请使用 Chrome 或 Edge。",
    voiceFailed: "语音识别失败，请检查麦克风权限后重试。",
    model: "模型",
  ...
  searchPage: {
    ...
    noEmbeddingModel: "您无法使用此功能，因为当前检索后端需要智能检索模型。请在模型页面设置一个嵌入模型，或在设置中切换检索后端。",
    ...
    searchType: "搜索类型",
    vectorSearchWarning: "语义检索需要智能检索模型。目前仅文本搜索可用。",
    textSearch: "文本搜索",
    vectorSearch: "语义检索",
    searchIn: "搜索范围",
    searchSources: "搜索来源",
  ...
  podcasts: {
    ...
    generateExplainerVideo: "同时生成讲解视频",
    generateExplainerVideoDesc: "脚本同时规划关键画面，图片生成后与真实语音时间轴合成为 MP4；不使用视频生成 API。",
    generating: "正在生成...",
    generate: "生成",
    ...
    explainerTaskStarted: "播客与讲解视频生成任务已开始",
    explainerVideo: "讲解视频",
    videoUnavailable: "讲解视频暂时无法加载。",
    videoGenerationFailed: "播客音频已完成，但讲解视频生成失败",
    keyframesCount: "共 {count} 个关键画面，按真实语音时间轴切换。",
  ...
  settings: {
    ...
    embeddingAndSearch: "智能检索与搜索",
    embeddingAndSearchDesc: "配置搜索和检索索引选项",
    embeddingBackend: "检索后端",
    embeddingBackendPlaceholder: "选择检索后端",
    embeddingApiBackend: "Embedding API",
    llmBm25Backend: "LLM 摘要索引 + BM25",
    embeddingBackendHelp: "Embedding API 使用向量模型构建语义索引，适合已有稳定检索模型的场景。LLM 摘要索引 + BM25 使用语言模型生成分块摘要、关键词和可回答问题，再结合 BM25、RRF 与模型重排进行检索；它不需要向量模型，但建立索引时会调用语言模型。",
    defaultEmbeddingOption: "默认检索索引选项",
    embeddingOptionPlaceholder: "选择检索索引选项",
    ask: "询问",
    always: "始终",
    never: "从不",
    embeddingHelp: "建立智能检索索引可以让您和 AI 助手更容易找到内容。如果您运行本地嵌入模型（如 Ollama），建议开启。对于在线服务商，请按处理量评估成本。",
    fileManagement: "文件管理",
  ...
  advanced: {
    ...
    rebuildEmbeddingsDesc: "为所有来源重建智能检索索引",
    currentVersion: "当前版本",
    latestVersion: "最新版本",
    status: "状态",
    updateAvailable: "版本 {version} 可用",
    updateAvailableDesc: "ForgeNote 的新版本可用。",
    upToDate: "已是最新",
    unknown: "未知",
    ...
    rebuild: {
      ...
      existing: "仅现有项",
      all: "全部项",
      existingDesc: "仅重建已有索引的项（速度较快，适用于切换模型）",
      allDesc: "重建已有项 + 为缺失索引的项补全（速度较慢，较全面）",
      include: "包含在重建中",
      ...
      whenToRebuildAns: "当您切换检索模型、升级模型版本、怀疑索引数据损坏或进行了大批量内容导入后，建议执行重建。",
      howLong: "重建需要多长时间？",
      howLongAns: "耗时取决于项目总数、模型速度和 API 速率限制。本地模型（如 Ollama）通常非常快。",
      isSafe: "在使用应用时重建安全吗？",
      isSafeAns: "是的，重建过程是安全的。它不会删除您的原始内容，仅会逐步替换检索索引。在大批量处理时，搜索速度可能会有轻微抖动。",
    },
  },
  transformations: {
    title: "内容转换规则",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "按学习用途配置 ForgeNote 的默认模型",
    missingRequiredModels: "缺少必需的模型：{models}。如果没有这些模型，ForgeNote 可能无法正常运行。",
    selectModelPlaceholder: "选择一个模型",
    requiredModelPlaceholder: "⚠️ 必需 - 请选择一个模型",
    ragModelLabel: "RAG 问答模型",
    ragModelDesc: "用于基于来源和笔记进行问答。",
    resourceSearchModelLabel: "资源搜索智能体模型",
    resourceSearchModelDesc: "用于规划 Web 搜索并重排学习资源。",
    learningAssetModelLabel: "通用资产生成模型",
    ...
    flashcardsModelDesc: "生成可翻阅的记忆卡片。",
    mindMapModelLabel: "知识导图",
    mindMapModelDesc: "生成 Mermaid 兼容的知识结构图。",
    readingModelLabel: "拓展阅读",
    readingModelDesc: "生成延伸阅读指南。",
    codeLabModelLabel: "代码实验",
    codeLabModelDesc: "生成实操代码案例和实验。",
    podcastScriptModelLabel: "播客脚本",
    podcastScriptModelDesc: "生成播客大纲和对话脚本。",
    embeddingModelLabel: "检索模型（Embedding）",
    embeddingModelDesc: "用于语义检索和智能检索索引",
    ttsModelLabel: "文字转语音模型",
    ttsModelDesc: "播客语音的默认/备用模型；角色配置中的语音模型会优先使用。",
    sttModelLabel: "语音转文字模型",
    sttModelDesc: "用于音频转录",
    embeddingChangeTitle: "检索模型变更",
    embeddingChangeConfirm: "您即将将检索模型从 {from} 更改为 {to}。",
    rebuildRequired: "重要提示：需要重建索引",
    rebuildReason: "更改检索模型需要重建所有现有检索索引以保持一致性。如果不重建，您的搜索可能会返回错误或不完整的结果。",
    whatHappensNext: "接下来会发生什么：",
    step1: "您的默认检索模型将被更新",
    step2: "在重新构建之前，现有检索索引将保持不变",
    step3: "新内容将使用新的检索模型",
    step4: "您应该尽快重新构建检索索引",
  ...
  apiKeys: {
    ...
    description: "将 API 密钥安全地存储在数据库中，以在 ForgeNote 中启用 AI 服务商。",
    encryptionRequired: "未配置加密密钥",
    encryptionRequiredDescription: "请将 FORGENOTE_ENCRYPTION_KEY 环境变量设置为任意密钥字符串，以启用将 API 密钥存储到数据库。",
    configured: "已配置",
    notConfigured: "未配置",
  ...
  setupBanner: {
    encryptionRequired: "未配置加密密钥",
    encryptionRequiredDescription: "请设置 FORGENOTE_ENCRYPTION_KEY 环境变量以启用安全凭据存储。",
