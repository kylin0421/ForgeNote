"""
Unified embedding utilities for ForgeNote.
...
def _get_embedding_batch_size() -> int:
    ...
    """
    raw = os.getenv("FORGENOTE_EMBEDDING_BATCH_SIZE", "10").strip()
    try:
        value = int(raw)
        if value < 1:
            raise ValueError
        ...
        logger.warning(
            "Invalid FORGENOTE_EMBEDDING_BATCH_SIZE='{}'; falling back to 10",
            raw,
        )
        return 10
