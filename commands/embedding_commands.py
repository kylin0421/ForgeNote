...
async def embed_note_command(input_data: EmbedNoteInput) -> EmbedNoteOutput:
    ...
    try:
        ...
        if await is_llm_bm25_backend():
            ...
            logger.info(
                f"Skipping vector embedding for note {input_data.note_id}; "
                "LLM-BM25 backend uses the note full-text index."
            )
...
async def embed_insight_command(input_data: EmbedInsightInput) -> EmbedInsightOutput:
    ...
    try:
        ...
        if await is_llm_bm25_backend():
            processing_time = time.time() - start_time
            logger.info(
                f"Skipping vector embedding for insight {input_data.insight_id}; "
                "LLM-BM25 backend uses source chunk metadata and text indexes."
            )
...
async def embed_source_command(input_data: EmbedSourceInput) -> EmbedSourceOutput:
    """
    Generate and store search index records for a source document.

    Creates multiple chunk records stored in the source_embedding table.
    Uses content-type aware chunking based on file extension or content heuristics.
    Depending on settings, records are either vector embeddings generated by an
    embedding API or LLM-enriched BM25 documents containing raw text, summaries,
    keywords, and hypothetical questions.
    ...
    5. Generate embeddings or LLM-BM25 metadata for all chunks
    ...
    try:
        ...
        if await is_llm_bm25_backend():
            logger.debug(f"Generating LLM-BM25 metadata for {total_chunks} chunks")
            records = await build_llm_bm25_source_records(
                source,
                chunks,
                command_id=cmd_id,
            )
            ...
            processing_time = time.time() - start_time
            logger.info(
                f"Successfully indexed source {input_data.source_id} with LLM-BM25: "
...
async def legacy_embed_single_item_command(
    ...
    try:
        await raise_if_command_canceled(get_command_id(input_data))
        logger.info(
...
async def rebuild_embeddings_command(
    ...
    try:
        ...
        llm_bm25_backend = await is_llm_bm25_backend()
        if llm_bm25_backend:
            language_model = await model_manager.get_default_model("retrieval")
            if not language_model:
                raise ValueError(
                    "No language model configured. Please configure a chat or transformation model in the Models section."
                )
            logger.info("LLM-BM25 backend configured; source jobs will build summary indexes")
        else:
        ...
        if llm_bm25_backend:
            logger.info("Skipping note vector jobs; LLM-BM25 uses note full-text search")
        else:
        ...
        if llm_bm25_backend:
            logger.info(
                "Skipping insight vector jobs; LLM-BM25 uses source chunk metadata and text search"
            )
        else:
