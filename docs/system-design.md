...
- 课程学习讲解：面向课程知识点的分层讲解。
- 博客式讲解：把“听不懂”的知识点转为问题驱动、直觉化的教学博客。
- 测验：用于快速诊断理解程度。
- 学习效果评估：结合实时画像、Quiz、错题和学习行为形成多维评估。
- 个性化学习路径：生成有顺序、资源、检查点与动态调整规则的学习路线。
- 闪卡：用于术语、公式和概念回忆。
- 思维导图：用于知识结构梳理。
- 拓展阅读：结合来源推荐延伸材料。
...
Studio 按钮与对话共享同一套 `generate_learning_asset` 工具。对话消息先经过确定性的意图路由：只有同时包含明确动作词和资产类型时才提交后台任务；例如“我听不懂，能不能给我来个博客讲解一下”会调用博客资产工具，普通的“这段代码是什么意思”仍保持为即时问答。

编辑和导出规则按资产类型控制：

- 只有思维导图和代码实验室中的 notebook 代码内容显示编辑入口。
- 课程学习讲解、博客式讲解、学习效果评估和个性化学习路径可导出文档。
- 测验通过错题本导出错题。
...
- 画像/事实分离：学习画像用于难度、偏好、评估和路径个性化，不作为课程事实证据。
- 引用可见：问答结果展示引用来源，学生可追踪依据。
- 提示注入隔离：来源文本被视为不可信数据，其中的角色设定、工具调用和越权指令不会执行。
- 安全状态可见：生成资产保存 trust 元数据，并在 Studio 卡片显示“来源校验”标记。
...
- 对话工具调用：聊天输入下方实时展示被调用的资产工具、当前智能体任务和完成百分比。
