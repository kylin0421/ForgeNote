...
4. 进入 `cs229` 学习记录。
5. 查看左侧资源搜索与学习画像。
6. 在右侧 Studio 查看课程讲解、博客讲解、测验、学习评估、学习路径、闪卡、思维导图、拓展阅读、代码实验、辅助图片和播客入口。
...
11. 测试导出规则：课程讲解、博客讲解、学习评估、学习路径、错题本、思维导图、代码实验室、播客有对应导出；拓展阅读和闪卡不显示导出。
12. 如果配置了 DashScope/Qwen `qwen-image`，在模型设置页测试图片模型连接。
13. 在对话中输入“我听不懂，能不能给我来个博客讲解一下”，确认出现工具调用进度并在 Studio 生成博客资产。
14. 输入普通问题“这段代码的第二行是什么意思”，确认不会误触发资产生成。
15. 生成学习效果评估和学习路径，确认内容引用实时画像；缺少 Quiz/实操证据时不显示虚假精确分数。

截图见 `docs/assets/`。

## 已知风险

- 讲解视频采用“播客脚本 + TTS + 关键画面 + 本地时间轴合成”方案，不依赖昂贵的视频生成 API；若要生成自由镜头动画，仍需外部视频模型。
